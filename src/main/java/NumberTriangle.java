...
public class NumberTriangle {
    ...
    public int retrieve(String path) {
        // If path is empty, retrieve the root of this triangle
        if (path.isEmpty()) {
            return this.root;
        }
        // Recursive call on left sub-triangle
        else if (path.charAt(0) == 'l') {
            return this.left.retrieve(path.substring(1));
        }
        // Recursive call on right sub-triangle
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        while (line != null) {
            ...
            NumberTriangle[] numbers  = new NumberTriangle[lineNum];
            String[] splitLine = line.split(" ");

            for (int i = 0; i < splitLine.length; i++) {
                numbers[i] = new NumberTriangle(Integer.parseInt(splitLine[i]));
            }

            for (int i = 0; i < prevRow.size(); i++) {
                NumberTriangle tri = prevRow.get(i);
                tri.setLeft(numbers[i]);
                tri.setRight(numbers[i + 1]);
            }

            prevRow = Arrays.asList(numbers);
